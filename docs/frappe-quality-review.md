...
> **Synced:** 2026-09-13
> **Maintained copy.** This is the in-repo source of truth for the Frappe framework rules distilled into Optimus's AI-fix prompt (`optimus/ai_fix.py`, the `_FRAPPE_REVIEW_RULES` constant). Re-sync this file when Frappe updates the upstream skill, then update `_FRAPPE_REVIEW_RULES` to match.
